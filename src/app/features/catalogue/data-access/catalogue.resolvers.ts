...
export const catalogueCardsResolver: ResolveFn<
  readonly CatalogueCategoryCard[]
> = () => inject(CatalogueRepository).getCategoryCards$();

export const catalogueCategoryResolver: ResolveFn<
  CatalogueCategory | RedirectCommand
> = (route) => {
  ...
  return repository
    ...
    .pipe(
      map(
        (category) =>
          category ?? new RedirectCommand(router.parseUrl('/not-found')),
      ),
    );
};

export const catalogueProductResolver: ResolveFn<
  CatalogueProduct | RedirectCommand
> = (route) => {
  const repository = inject(CatalogueRepository);
  const router = inject(Router);
  ...
  return repository
    ...
    .pipe(
      map(
        (product) =>
          product ?? new RedirectCommand(router.parseUrl('/not-found')),
      ),
    );
};

export const catalogueCategoryTitleResolver: ResolveFn<string> = (route) =>
  inject(CatalogueRepository).getCategoryTitle$(
    route.paramMap.get('category') ?? '',
  );
...
export const catalogueCategoryDescriptionResolver: ResolveFn<string> = (
  route,
) =>
  inject(CatalogueRepository).getCategoryDescription$(
    route.paramMap.get('category') ?? '',
  );
